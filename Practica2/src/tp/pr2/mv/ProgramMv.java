...
public class ProgramMv {
	...
	/**
	 * Añade la expresión pasada al programa Muestra el equivalente en
	 * ensamblador
	 * 
	 * @param instr
	 *            : instrucción a añadir
	 */
	public void pushExp(Expresion instr) {
		while (this.contMax <= this.cont + instr.getSetOfInstr().size()) {
			redimensiona();
		}

		for (int i = 0; i < instr.getSetOfInstr().size(); i++) {
			System.out.println(instr.getSetOfInstr().get(i).toString());
			this.program[this.cont++] = instr.getSetOfInstr().get(i);
		}
	}
